...
import { ArrowLeft, Plus, Save, X, GripVertical, Clock, User as UserIcon, Package, Trash2 } from 'lucide-react';
import { api } from '@/lib/api';
import { Recipe, RawMaterial, User, RecipeIngredient, RecipeStep, StepTaskTemplate, TaskType, TaskPriority } from '@/types';
...
export default function EditRecipePage() {
  ...
  const fetchInitialData = async () => {
    try {
      const [recipeRes, rawMaterialsRes, usersRes] = await Promise.all([
      ...
      if (recipeRes.ok) {
        ...
        setForm({
          ...
          recipeProcess: recipe.recipeProcess || { name: '', description: '', steps: [] }
        });
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(error.error || 'Failed to update recipe');
      }
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const error = await response.json();
        toast.error(error.error || 'Failed to delete recipe');
      }
    } catch (error) {
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-2">
                Update recipe details, ingredients, and cooking process
              </p>
            </div>
            <Button
              variant="destructive"
